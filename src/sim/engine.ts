import type { EngineParams, Snapshot } from "./types";
...
export class Engine {
  ...
  // runtime switches
  paused = false; // pauses entire tick
  violence = true; // gates kill() only
  expansion = true; // NEW: gates radius growth + spawning stars/civs
  ...
  reset() {
    this.stepN = 0;
    this.time = 0;
    this.radius = this.params.radiusStart;
    ...
    // Note: do NOT change runtime flags (paused/violence/expansion) on reset
  }

  private randRange(r: number) {
    return (this.rng() * 2 - 1) * r;
  }
  ...
  private spawnCiv() {
    ...
    const s = this.rng();
    this.civStrat[this.civCount] =
      s < this.params.mix.silent
        ? 0
        : s < this.params.mix.silent + this.params.mix.broadcast
        ? 1
        : s <
          this.params.mix.silent +
            this.params.mix.broadcast +
            this.params.mix.cautious
        ? 2
        : 3;
  ...
  private detect(a: number, b: number) {
    ...
    const ax = this.civPos[a * 3],
      ay = this.civPos[a * 3 + 1],
      az = this.civPos[a * 3 + 2];
    const bx = this.civPos[b * 3],
      by = this.civPos[b * 3 + 1],
      bz = this.civPos[b * 3 + 2];
    ...
    const p =
      this.params.pDetectBaseMin +
      (this.params.pDetectBaseMax - this.params.pDetectBaseMin) *
        Math.exp(-range * this.params.rDetectBase);
    return this.rng() < p;
  }
   ...
   *
   * Expansion OFF:
   *  - radius does NOT grow
   *  - NO new stars or civs spawn
   *  - existing civs still run detection (and kills if violence enabled)
   */
  step() {
    ...
    if (this.expansion) {
      ...
      // spawn stars proportionally to volume
      const vol = (4 / 3) * Math.PI * this.radius ** 3;
      const desiredStars = Math.min(
        this.params.maxStars,
        Math.floor(vol * this.params.starDensityPerVol)
      );
      while (this.starCount < desiredStars) this.spawnStar();

      // probabilistic civ spawns
      if (this.rng() < this.params.civSpawnProb) this.spawnCiv();
    }

    // ----- Interactions among existing civs -----
    for (let i = 0; i < this.civCount; i++)
      if (this.civAlive[i]) {
        for (let j = 0; j < this.civCount; j++)
          if (i !== j && this.civAlive[j]) {
            if (this.detect(i, j)) {
              this.revealsS++;
  ...
  snapshot(): Snapshot {
    let alive = 0;
    for (let i = 0; i < this.civCount; i++) if (this.civAlive[i]) alive++;
    return {
      step: this.stepN,
      time: this.time,
      radius: this.radius,
